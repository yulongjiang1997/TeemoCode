...
describe("提示音双向同步", () => {
  it("音效页初值来自 sound_enabled;切换发 set_sound_enabled;壳广播回来盖一次", async () => {
    const { calls, listeners } = stubShell({ sound: false });
    render(<SettingsView onClose={() => {}} />);
    // 初始分区是「账号」,先切到音效
    await userEvent.click(screen.getByRole("button", { name: "音效" }));
    const toggle = () => screen.getByRole("checkbox", { name: "全局提示音" }) as HTMLInputElement;
